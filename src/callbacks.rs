...
pub(crate) trait AnyCallback {
    ...
    fn run(&mut self, _: &Discord);
}

pub(crate) struct ResultCallback<F>
where
    F: FnMut(&Discord, Result<()>),
{
    ...
    F: FnMut(&Discord, Result<()>),
{
    ...
    fn run(&mut self, discord: &Discord) {
