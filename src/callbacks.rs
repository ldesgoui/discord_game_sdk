...
pub(crate) trait AnyCallback {
    ...
    fn run(&mut self, _: &Discord<'_>);
}

pub(crate) struct ResultCallback<F>
where
    F: FnMut(&Discord<'_>, Result<()>),
{
    ...
    F: FnMut(&Discord<'_>, Result<()>),
{
    ...
    fn run(&mut self, discord: &Discord<'_>) {
...
where
    F: FnMut(&Discord<'_>, Result<String>),
{
    pub(crate) callback: F,
...
where
    F: FnMut(&Discord<'_>, Result<Vec<u8>>),
{
    pub(crate) callback: F,
